...
// Use the 2026 Excel file inside the workspace
const excelPath = path.resolve(__dirname, '..', 'BANCOS 2026.xlsx');
...
// Parse date and force the year to 2026
function parseExcelDate(val) {
  ...
  if (d.getUTCFullYear() !== 2026) {
    d = new Date(Date.UTC(2026, d.getUTCMonth(), d.getUTCDate()));
  }
...
async function processSheet(config, accounts) {
  ...
  // Limpiar movimientos anteriores del 2026 de esta cuenta (No tocar los del 2025)
  const { error: delErr } = await supabase.from('movimientos')
    .delete()
...
async function run() {
  console.log('🚀 Importando BANCOS 2026.xlsx...\n');
  ...
  console.log('\n✨ Importación 2026 completada exitosamente.');
}
