        ...
        try {
            ...
            if (data.success) {
                ...
                alert(
                    ...
                    `📧 Correos procesados: ${data.sync}\n` +
                    `🔗 Facturas vinculadas automáticamente: ${matched}\n` +
                    `💡 Con sugerencias para revisar: ${suggested}`
                );
                fetchFacturas();
            } else {
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Total Sincronizadas</p>
                ...
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-2xl font-black text-zinc-600 dark:text-zinc-400">{counts.descartada}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full pl-10 pr-10 py-4 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 placeholder:text-zinc-400 border border-zinc-100 dark:border-zinc-900 rounded-2xl text-sm focus:ring-2 focus:ring-primary/20 shadow-sm transition-all"
                        value={searchTerm}
                        onChange={(e) => {
                    ...
                    {searchTerm && (
                        <button 
                            ...
                            className="absolute right-4 top-1/2 -translate-y-1/2 p-1 bg-zinc-100 dark:bg-zinc-900 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 rounded-full text-xs"
                        >
            ...
            <div className="bg-white dark:bg-zinc-950 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-900 overflow-hidden shadow-sm">
                        ...
                        <tbody className="divide-y divide-zinc-50 dark:divide-zinc-900">
                            {paginatedFacturas.map((factura) => (
                                <tr 
                                    ...
                                    className="hover:bg-zinc-50/40 dark:hover:bg-zinc-900/20 transition-colors cursor-pointer"
                                >
                                    <td className="px-6 py-5">
                                        ...
                                        <p className="text-[10px] text-zinc-400 font-medium">{safeFormatDate(factura.fecha_emision, "yyyy")}</p>
                                    </td>
                                    <td className="px-6 py-5">
                                        <p className="text-xs font-black text-zinc-900 dark:text-zinc-50 uppercase">
                                            {highlightMatch(factura.emisor_nombre, searchTerm)}
                                        </p>
                                        <p className="text-[10px] text-zinc-400 font-mono flex gap-2">
                                    ...
                                    <td className="px-6 py-5 text-center" onClick={(e) => e.stopPropagation()}>
                                        {factura.estado === 'VINCULADA' ? (
                                            ...
                                            <span className="px-3 py-1 rounded-full bg-zinc-500/10 text-zinc-600 text-[9px] font-black uppercase tracking-widest border border-zinc-500/20">Excluida</span>
                                    ...
                                    <td className="px-6 py-5">
                                        {factura.estado === 'VINCULADA' && factura.movimientos ? (
                                            ...
                                            <p className="text-[10px] text-zinc-400 italic font-medium">Descartada de conciliación</p>
                                        ) : (factura.moneda === 'XXX' || factura.monto_total === 0) ? (
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            {factura.estado !== 'VINCULADA' && factura.estado !== 'DESCARTADA' ? (
                                                ...
                                                <button 
                                                    ...
                                                    className="p-2 rounded-xl bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-all flex items-center gap-1.5 text-[9px] font-black uppercase tracking-wider"
                                                >
                                            ...
                                            <button 
                                                onClick={() => setDetailInvoice(factura)}
                                                className="p-2 rounded-xl bg-zinc-50 dark:bg-zinc-900 text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors"
                                            >
                            ...
                            {filteredFacturas.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="text-center py-20">
                                        ...
                                        <p className="text-zinc-400 text-sm font-medium">No se encontraron facturas con los filtros seleccionados.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                {/* Pagination */}
                {totalPages > 1 && (
                    <div className="px-6 py-5 bg-zinc-50/30 dark:bg-zinc-900/30 border-t border-zinc-100 dark:border-zinc-900 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            <button 
                                ...
                                className="p-2 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-900 text-zinc-400 disabled:opacity-30"
                            >
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between pb-6 border-b border-zinc-100 dark:border-zinc-800">
                                <div>
                                    ...
                                    <p className="text-zinc-400 text-xs font-mono break-all mt-1">{detailInvoice.uuid_sat}</p>
                                </div>
                                <button 
                                    onClick={() => setDetailInvoice(null)}
                                    className="p-2 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 dark:text-zinc-500 hover:text-zinc-800 transition-colors"
                                >
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 py-8 space-y-6">
                                {/* Amount section */}
                                <div className="text-center p-6 rounded-3xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-900">
                                    <p className="text-xs font-black uppercase text-zinc-400 tracking-wider">Monto Total del XML</p>
                                    <p className="text-4xl font-black text-zinc-900 dark:text-zinc-50 mt-1">
                                        ${parseFloat(detailInvoice.monto_total).toLocaleString()}
                                        <span className="text-sm font-bold text-zinc-400 ml-1.5 uppercase">{detailInvoice.moneda}</span>
                                    </p>
                                ...
                                <div className="space-y-3">
                                    <div className="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 flex items-start gap-3.5 shadow-xs">
                                        <div className="p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-950 text-zinc-400">
                                            <Building2 className="w-5 h-5" />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-400 uppercase tracking-wide">Emisor / Proveedor</p>
                                    ...
                                    <div className="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 flex items-start gap-3.5 shadow-xs">
                                        ...
                                        <div>
                                            <p className="text-[10px] font-black text-zinc-400 uppercase tracking-wide">Receptor / Empresa</p>
                                ...
                                <div className="space-y-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                                    <h3 className="text-xs font-black uppercase text-zinc-400 tracking-wider">Datos Fiscales</h3>
                                    <div className="grid grid-cols-2 gap-4 text-xs">
                                        ...
                                        <div className="col-span-2">
                                            ...
                                            <p className="font-mono text-zinc-600 dark:text-zinc-450 bg-zinc-50 dark:bg-zinc-950 p-2 rounded-lg border border-zinc-100 dark:border-zinc-900 mt-1 select-all">{detailInvoice.uuid_sat}</p>
                                        </div>
                                        <div className="col-span-2">
                                            <p className="text-zinc-400">Archivo XML Sincronizado</p>
                                            <div className="flex items-center gap-2 mt-1.5 text-zinc-550 dark:text-zinc-350">
                                ...
                                <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800 space-y-3">
                                    <h3 className="text-xs font-black uppercase text-zinc-400 tracking-wider">Vínculo Bancario</h3>
                                    {detailInvoice.estado === 'VINCULADA' && detailInvoice.movimientos ? (
                                        <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 space-y-3 shadow-xs">
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-zinc-400 font-bold uppercase mt-1.5 flex gap-2">
                                        ...
                                        <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-900 space-y-4 text-center">
                                            {detailInvoice.estado === 'DESCARTADA' ? (
                                                <>
