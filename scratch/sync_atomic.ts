...
async function atomicSync() {
    ...
    for (const cfg of ACCOUNTS) {
        ...
        let b = 0; // Mathematical sum
        let lastE = 0; // Last encountered Excel saldo
        let lastD = '2026-01-01';
        let startIdx = -1;

        // 1. Find the STARTING ROW (first non-zero Saldo)
        for (let i = 0; i < d.length; i++) {
            const row = d[i];
            const s = parse(row[m[7]]);
            if (s !== 0) {
                ...
                const iso = getISO(row[m[0]]);
                if (iso) lastD = iso;

                // Adjust balance back if this row also has a movement (to keep start clear)
                let startMonto = s;
                if (!label.includes("INICIAL") && (ing !== 0 || egr !== 0)) {
                    startMonto = s - (ing - egr);
                    b = startMonto;
                }

                megaPayload.push({
                    id: `00000000-0000-0000-0000-${megaPayload.length.toString().padStart(12, '0')}`,
                    cuenta_id: cfg.id,
                    fecha: lastD,
                ...
                console.log(`  Row ${i+1} START: ${startMonto}`);
                break;
            }
        }

        if (startIdx === -1) { console.log('  No data found'); continue; }

        // 2. Process all rows from startIdx onwards
        ...
        // 3. Close the gap (Drift Correction)
        const drift = lastE - b;
    ...
    console.log(`\nFinalizing: ${megaPayload.length} rows.`);
    const BATCH = 100;
    for (let i = 0; i < megaPayload.length; i += BATCH) {
        const { error } = await supabase.from('movimientos').insert(megaPayload.slice(i, i + BATCH));
