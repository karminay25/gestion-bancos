...
function getISO(s: any) {
    if (typeof s !== 'number') return '2026-01-01';
    return new Date((s - 25569) * 86400000).toISOString().split('T')[0];
}

async function finalSyncV9() {
    console.log('=== ATOMIC PRECISION SYNC V9 (DEFINITIVE) ===');
    ...
    for (const c of ACCOUNTS) {
        ...
        // 1. Find the First Row with a Numeric Saldo
        for (let i = 0; i < d.length; i++) {
            const row = d[i];
            const sVal = parse(row[m[7]]);
            if (typeof row[m[7]] === 'number' && sVal !== 0) {
                bSum = sVal;
                lastE = sVal;
                startIdx = i;
                ...
                pay.push({
                    ...
                    cuenta_id: c.id,
                    fecha: getISO(row[m[0]]),
                    nombre_tercero: (row[m[3]]?.toString() || 'SALDO INICIAL').toUpperCase(),
                    monto: sVal,
                    tipo: 'Ingreso',
        ...
        // 2. Process all rows after the start
        for (let i = startIdx + 1; i < d.length; i++) {
            ...
            if (ing === 0 && egr === 0) {
                if (typeof row[m[7]] === 'number' && sVal !== 0) lastE = sVal;
                continue;
            }
            ...
            bSum += (tipo === 'Ingreso' ? monto : -monto);
            lastE = (typeof row[m[7]] === 'number' && sVal !== 0) ? sVal : lastE;
        ...
        const drift = lastE - bSum;
        if (Math.abs(drift) > 0.01) {
            console.log(`  [DRIFT] ${c.name}: ${drift.toFixed(2)}. Adding Correction.`);
            pay.push({
                id: `00000000-0000-0000-0000-${pay.length.toString().padStart(12, '0')}`,
                cuenta_id: c.id,
                fecha: pay[pay.length-1].fecha,
                nombre_tercero: 'AJUSTE EXCEL',
                monto: Math.abs(drift),
                tipo: drift > 0 ? 'Ingreso' : 'Egreso',
                concepto: 'Ajuste final para coincidir 100% con el saldo reportado en Excel.'
            });
        }
    }

    console.log(`\nFinalizing ${pay.length} records...`);
    const BATCH = 100;
