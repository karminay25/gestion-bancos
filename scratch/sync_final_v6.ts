...
async function definitiveSyncV6() {
    console.log('=== DEFINITIVE PRECISION SYNC V6 (MULTI-YEAR SUPPORT) ===');
    
    console.log('Cleaning movements table...');
    await supabase.from('movimientos').delete().match({}); // Matching nothing is dangerous but PostgREST filter 'match({})' deletes all if RLS allows
    // Better safe wipe:
    ...
    for (const [sheetName, accountId] of Object.entries(ACCOUNTS)) {
        ...
        // Find the first valid date serial to determine the year of the "Saldo Inicial"
        let firstDetectedDate: string | null = null;
        for (let i = 7; i < data.length; i++) {
            const d = getISO(data[i][0]);
            if (d) {
                firstDetectedDate = d;
                break;
            }
        ...
        // If no date found, default to 2026-01-01
        const initialDate = firstDetectedDate || '2026-01-01';

        // Row 7 (index 6) is Initial Saldo
        const r7 = data[6];
        if (r7 && r7[2] === 'SALDO INICIAL') {
            const ini = parseNum(r7[5]);
            payload.push({
                id: `00000000-0000-0000-0000-${payload.length.toString().padStart(12, '0')}`,
                cuenta_id: accountId,
                fecha: initialDate, // Use the natural start date of the sheet
                nombre_tercero: 'SALDO INICIAL',
                monto: ini,
                tipo: 'Ingreso',
                concepto: 'Balance Inicial'
            });
            if (sheetName === 'BBVA PESOS') bbvaCheck = ini;
        }

        let lastDate: string | null = initialDate;

        for (let i = 7; i < data.length; i++) {
            const row = data[i];
            if (!row || row.length < 3) continue;

            const currentDate = getISO(row[0]);
            if (currentDate) {
                lastDate = currentDate;
            } else if (typeof row[0] === 'string' && row[0].trim() !== '') {
                // Ignore Month Title rows
                continue;
            }

            const ing = parseNum(row[3]);
            const egr = parseNum(row[4]);
            if (ing === 0 && egr === 0 && !row[2]) continue;
            ...
            if (sheetName === 'BBVA PESOS') {
                bbvaCheck += (tipo === 'Ingreso' ? monto : -monto);
            }
            ...
            payload.push({
                ...
                cuenta_id: accountId,
                fecha: lastDate,
                nombre_tercero: row[2]?.toString() || 'S/N',
                monto,
                tipo,
                factura: row[7]?.toString() || null,
                concepto: row[8]?.toString() || null,
                centro_costo_id: CC[row[9]?.toString().trim() as keyof typeof CC] || null
            });
        }
    }

    console.log(`Verification: BBVA PESOS Final Balance = $${bbvaCheck.toLocaleString()}`);
