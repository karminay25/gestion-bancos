...
async function finalPrecisionSync() {
    console.log('=== ULTIMATE PRECISION SYNC ===');
    ...
    for (const cfg of ACCOUNTS) {
        ...
        const m = cfg.cols; // [Fecha, Ing, Egr, Nom, Fac, Con, CC, Saldo]
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // Start detection
            if (!foundStart) {
                if (nombre.includes('SALDO INICIAL') || (cfg.name === 'CREDITO BAJIO' && saldoCol !== 0 && i > 0)) {
                    runningSum = saldoCol;
        ...
        if (Math.abs(drift) > 0.01) {
            ...
            payload.push({
                ...
                concepto: 'Ajuste para coincidir con el saldo del Excel'
            });
        }
    }
