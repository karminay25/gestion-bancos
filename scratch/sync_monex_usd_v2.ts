...
function getISO(s: any) {
    ...
    // Fix 2024 typo to 2025 as per Exercise context
    if (d.getFullYear() === 2024) d.setFullYear(2025);
    return d.toISOString().split('T')[0];
}

async function syncMonexUSD() {
    console.log(`=== SYNC DEFINITIVE V2: ${SHEET_NAME} ===`);
    ...
    // 1. Find and set Saldo Inicial in account table
    let initialBalance = 0;
    let startIdx = -1;
    for (let i = 0; i < d.length; i++) {
        const row = d[i];
        const nombre = row[m[3]]?.toString().toUpperCase() || "";
        if (nombre.includes('SALDO INICIAL')) {
            initialBalance = parse(row[m[7]]);
            startIdx = i;
            break;
        }
    }
    ...
    if (startIdx === -1) {
        console.error('Could not find SALDO INICIAL row');
        return;
    }

    console.log(`Setting Saldo Inicial to: ${initialBalance}`);
    const { error: accErr } = await supabase.from('cuentas_bancarias').update({ saldo_inicial: initialBalance }).eq('id', ACCOUNT_ID);
    if (accErr) { console.error('Error updating account:', accErr); return; }
    ...
    console.log('Account movements cleared.');

    const payload: any[] = [];
    let mathBalance = initialBalance;
    let lastExcelSaldo = initialBalance;
    ...
    for (let i = startIdx + 1; i < d.length; i++) {
        ...
        payload.push({
            id: `00000000-0000-0000-mon3-${payload.length.toString().padStart(12, '0')}`,
