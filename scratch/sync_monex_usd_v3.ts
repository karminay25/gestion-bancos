...
function getISO(s: any) {
    ...
    // Convert 2024 to 2025 to match exercise context
    if (d.getFullYear() === 2024) d.setFullYear(2025);
    return d.toISOString().split('T')[0];
}

async function syncMonexUSD() {
    console.log(`=== SYNC DEFINITIVE V3: ${SHEET_NAME} ===`);
    ...
    for (let i = 0; i < d.length; i++) {
        ...
        if (!started) {
            if (saldo !== 0 || nombre.toUpperCase().includes('SALDO INICIAL')) {
                ...
                // Insert Saldo Inicial as a movement with a VERY EARLY date so it's always first
                payload.push({
                    id: `00000000-0000-0000-mon3-000000000000`,
                    cuenta_id: ACCOUNT_ID,
                    fecha: '2025-01-01', // Starting date of exercise
                    nombre_tercero: 'SALDO INICIAL',
                    monto: startMonto,
                    tipo: 'Ingreso',
                    concepto: 'Balance de Apertura (Excel)',
                    created_at: '2025-01-01T00:00:00Z' // Ensure it's first in same-day sorting
                });
    ...
    if (Math.abs(drift) > 0.01) {
        payload.push({
            id: `00000000-0000-0000-mon3-ffffffffffff`,
            cuenta_id: ACCOUNT_ID,
            fecha: lastDate,
