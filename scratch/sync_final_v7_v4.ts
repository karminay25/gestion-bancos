...
async function finalPrecisionSync() {
    console.log('=== ULTIMATE ROBUST PRECISION SYNC ===');
    ...
    for (const cfg of ACCOUNTS) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const ing = parseNum(row[m[1]]);
            const egr = parseNum(row[m[2]]);
            const saldoCol = parseNum(row[m[7]]);

            // ROBUST START: First row with a non-zero Saldo
            if (!foundStart) {
                if (saldoCol !== 0) {
                    runningSum = saldoCol;
                    lastExcelSaldo = saldoCol;
                    foundStart = true;
                    payload.push({
                    ...
                    console.log(`  [START] Row ${i+1} Balance: ${saldoCol}`);
                    
                    // If this row also has a movement, process it too
                    if (ing !== 0 || egr !== 0) {
                        const monto = ing || egr;
                        const tipo = ing > 0 ? 'Ingreso' : 'Egreso';
                        runningSum = saldoCol - (tipo === 'Ingreso' ? monto : -monto); 
                        // Wait, if it's the start row, the saldo ALREADY includes the movement usually.
                        // UNLESS it's a "SALDO INICIAL" label row.
                        const label = row[m[3]]?.toString().toUpperCase() || "";
                        if (label.includes('SALDO INICIAL')) {
                            // Perfect, it's just a starting balance row.
                            runningSum = saldoCol;
                        } else {
                            // It's a movement row that happens to be the first one with a saldo.
                            // We should probably subtract the movement to get the "pre-movement" start.
                            // BUT simpler: let's just count it as the start and the movement will follow in next logic?
                            // No, if we 'continue', we lose the movement.
                            // Adjusting:
                            const preMoveBalance = saldoCol - (tipo === 'Ingreso' ? monto : -monto);
                            payload[payload.length-1].monto = preMoveBalance;
                            runningSum = preMoveBalance;
                            // Now fall through to normal processing to add the movement.
                        }
                    } else {
                       continue;
                    }
            ...
            if (monto !== 0) {
                ...
                lastExcelSaldo = saldoCol || lastExcelSaldo;
        ...
        if (Math.abs(drift) > 0.01) {
            console.log(`  [DRIFT] $${drift.toFixed(2)}. Adding Adjustment.`);
            payload.push({
                id: `00000000-0000-0000-0000-${payload.length.toString().padStart(12, '0')}`,
                cuenta_id: cfg.id,
                fecha: lastDate,
