...
async function theFinalFinalFinalSync() {
    console.log('=== THE TERMINAL PRECISION SYNC ===');
    ...
    for (const c of ACCOUNTS) {
        ...
        for (let i = 0; i < d.length; i++) {
            ...
            if (typeof row[m[7]] === 'number' && sVal !== 0) {
                ...
                mega.push({
                    id: `00000000-0000-0000-0000-${mega.length.toString().padStart(12, '0')}`,
                    cuenta_id: c.id,
                    fecha: lastD,
                ...
                console.log(`  Start Row ${i+1} Set: ${sVal}`);
                break;
            }
        }
        ...
        if (Math.abs(drift) > 0.01) {
            ...
            mega.push({
                ...
                concepto: 'Ajuste de precisión final'
            });
        }
    }

    console.log(`\nInserting ${mega.length} records...`);
    const BATCH = 100;
    for (let i = 0; i < mega.length; i += BATCH) {
        await supabase.from('movimientos').insert(mega.slice(i, i + BATCH));
    }
    console.log('=== MISSION ACCOMPLISHED ===');
}

theFinalFinalFinalSync();
