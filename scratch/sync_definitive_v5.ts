import { createClient } from '@supabase/supabase-js';
import XLSX from 'xlsx';
import dotenv from 'dotenv';
dotenv.config({ path: '.env.local' });

const EXCEL_PATH = 'c:/proyectoResidencias/BANCOS 2026.xlsx';
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
);

const ACCOUNTS = {
  'BBVA PESOS': '92e326e1-77a6-4426-95f6-505f0b36d852',
  'BBVA DOLARES': 'b865af21-de39-4dbc-bf37-245c49a8ce50',
  'MONEX USD': '3690f38f-6ea9-46de-b8cc-e183e0542ab5',
  'BAJIO PESOS': 'dbc8cc6e-b89a-4b37-ac42-919bce678ea8',
  'BAJIO USD': '16806e60-2b77-48c8-98b6-3e40c0505247',
  'BOSBES PESOS BBVA': 'cccb5951-8232-4ab7-b9cf-47b918677999',
  'BOSBES USD BBVA ': '64d7c5b4-f28e-4117-bcb8-d3731c1af35b',
  'BOSBES USD MONEX': '62d2bb5c-5f31-4f33-835d-6939e92f8485'
};

const CC = {
  'LOLA': '6a579200-6474-40a2-9e62-325bb63cd132',
  'OBA': '5353b861-04c2-4d1a-bf3d-318a44b4e87c',
  'BOSBES': '781e0438-b6a4-4a57-9c24-65334c68f123',
  'SOCIO CARLOS': '1916c7cb-9e43-4a8a-a3e3-8b3bc7bb82d1',
  'SOCIO JOSE': '4ec14c0c-1a1f-43c0-9c0d-e096d8dc3713',
  'SOCIO LUIS': '2fbe4ba1-23a6-4ba4-98ef-cd2e2f68115d',
  'CRFV': '45688ff0-f68e-4c28-8e5f-0421578eee44'
};

function parseNum(v: any) {
    if (typeof v === 'number') return v;
    if (typeof v === 'string') return parseFloat(v.replace(/[\$,\s]/g, '')) || 0;
    return 0;
}

function getISO(s: any) {
    if (typeof s !== 'number') return null;
    return new Date((s - 25569) * 86400000).toISOString().split('T')[0];
}

async function definitiveSyncV5() {
    console.log('=== DEFINITIVE PRECISION SYNC V5 (TOTAL WIPE) ===');
    
    // Clear ALL movements by matching everything
    console.log('Cleaning table...');
    const { error: delError } = await supabase.from('movimientos').delete().gte('monto', 0);
    if (delError) {
        console.error('Initial cleanup failed:', delError.message);
    }
    
    const wb = XLSX.readFile(EXCEL_PATH);
    const payload: any[] = [];
    let bbvaCheck = 0;

    for (const [sheetName, accountId] of Object.entries(ACCOUNTS)) {
        const sheet = wb.Sheets[sheetName];
        if (!sheet) continue;
        const data: any[][] = XLSX.utils.sheet_to_json(sheet, { header: 1 });
        
        const r7 = data[6];
        if (r7 && r7[2] === 'SALDO INICIAL') {
            const ini = parseNum(r7[5]);
            payload.push({
                id: `00000000-0000-0000-0000-${payload.length.toString().padStart(12, '0')}`,
                cuenta_id: accountId,
                fecha: '2026-01-01',
                nombre_tercero: 'SALDO INICIAL',
                monto: ini,
                tipo: 'Ingreso',
                concepto: 'Balance Anterior'
            });
            if (sheetName === 'BBVA PESOS') bbvaCheck = ini;
        }

        let lastDate: string | null = '2026-01-01';
        for (let i = 7; i < data.length; i++) {
            const row = data[i];
            if (!row || row.length < 3) continue;
            const currentDate = getISO(row[0]);
            if (currentDate) lastDate = currentDate;
            else if (typeof row[0] === 'string' && row[0].trim() !== '') continue;

            const ing = parseNum(row[3]);
            const egr = parseNum(row[4]);
            if (ing === 0 && egr === 0 && !row[2]) continue;

            const monto = ing || egr;
            if (monto === 0) continue;

            const tipo = ing > 0 ? 'Ingreso' : 'Egreso';
            if (sheetName === 'BBVA PESOS') bbvaCheck += (tipo === 'Ingreso' ? monto : -monto);

            payload.push({
                id: `00000000-0000-0000-0000-${payload.length.toString().padStart(12, '0')}`,
                cuenta_id: accountId,
                fecha: lastDate,
                nombre_tercero: row[2]?.toString() || 'S/N',
                monto,
                tipo,
                factura: row[7]?.toString() || null,
                concepto: row[8]?.toString() || null,
                centro_costo_id: CC[row[9]?.toString().trim() as keyof typeof CC] || null
            });
        }
    }

    console.log(`Verification: BBVA PESOS Final Balance = $${bbvaCheck.toLocaleString()}`);
    console.log(`Payload size: ${payload.length}`);

    const BATCH = 50;
    for (let i = 0; i < payload.length; i += BATCH) {
        const batch = payload.slice(i, i + BATCH);
        const { error } = await supabase.from('movimientos').insert(batch);
        if (error) console.error(`Batch ${i} failed:`, error.message);
        else process.stdout.write('.');
    }
    console.log('\n=== SYNC COMPLETE ===');
}

definitiveSyncV5();
