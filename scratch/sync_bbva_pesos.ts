...
async function syncBBVA() {
    ...
    for (let i = 0; i < d.length; i++) {
        ...
        // Start when we find the first non-zero saldo or "SALDO INICIAL"
        if (!started) {
            if (saldo !== 0 || nombre.toUpperCase().includes('SALDO INICIAL')) {
                started = true;
                let startMonto = saldo;
                
                // If it's a movement row that also defines the start, backtrack balance
                ...
                payload.push({
                    id: `00000000-0000-0000-bba1-${payload.length.toString().padStart(12, '0')}`,
                ...
                // If it was just the saldo inicial row, we skip adding it as a movement again
