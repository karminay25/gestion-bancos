import { createClient } from '@supabase/supabase-js';
import XLSX from 'xlsx';
import dotenv from 'dotenv';

dotenv.config({ path: '.env.local' });

const EXCEL_PATH = 'c:/proyectoResidencias/BANCOS 2026.xlsx';
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
);

const ACCOUNTS = {
  'BBVA PESOS': '92e326e1-77a6-4426-95f6-505f0b36d852',
  'BBVA DOLARES': 'b865af21-de39-4dbc-bf37-245c49a8ce50',
  'MONEX USD': '3690f38f-6ea9-46de-b8cc-e183e0542ab5',
  'BAJIO PESOS': 'dbc8cc6e-b89a-4b37-ac42-919bce678ea8',
  'BAJIO USD': '16806e60-2b77-48c8-98b6-3e40c0505247',
  'BOSBES PESOS BBVA': 'cccb5951-8232-4ab7-b9cf-47b918677999',
  'BOSBES USD BBVA ': '64d7c5b4-f28e-4117-bcb8-d3731c1af35b',
  'BOSBES USD MONEX': '62d2bb5c-5f31-4f33-835d-6939e92f8485'
};

const CC_MAP: Record<string, string> = {
  'LOLA': '6a579200-6474-40a2-9e62-325bb63cd132',
  'OBA': '5353b861-04c2-4d1a-bf3d-318a44b4e87c',
  'BOSBES': '781e0438-b6a4-4a57-9c24-65334c68f123',
  'SOCIO CARLOS': '1916c7cb-9e43-4a8a-a3e3-8b3bc7bb82d1',
  'SOCIO JOSE': '4ec14c0c-1a1f-43c0-9c0d-e096d8dc3713',
  'SOCIO LUIS': '2fbe4ba1-23a6-4ba4-98ef-cd2e2f68115d'
};

function parseValue(val: any): number {
    if (typeof val === 'number') return val;
    if (typeof val === 'string') {
        const clean = val.replace(/[\$,\s]/g, '');
        return parseFloat(clean) || 0;
    }
    return 0;
}

function excelDate(serial: any) {
    if (!serial || isNaN(serial)) return null;
    const date = new Date((serial - 25569) * 86400 * 1000);
    return date.toISOString().split('T')[0];
}

async function syncFinal() {
    console.log('--- FINAL PRECISION SYNC ---');
    await supabase.from('movimientos').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    
    const wb = XLSX.readFile(EXCEL_PATH);
    const payload: any[] = [];
    let bbvaBalance = 0;

    for (const [sheetName, accountId] of Object.entries(ACCOUNTS)) {
        const sheet = wb.Sheets[sheetName];
        if (!sheet) {
            console.error(`Sheet ${sheetName} not found!`);
            continue;
        }

        const data: any[][] = XLSX.utils.sheet_to_json(sheet, { header: 1 });
        
        // Initial Saldo at row 7 (index 6)
        const row7 = data[6];
        if (row7 && row7[2] === 'SALDO INICIAL') {
            const ini = parseValue(row7[5]);
            payload.push({
                cuenta_id: accountId,
                fecha: '2026-01-01',
                nombre_tercero: 'SALDO INICIAL',
                monto: ini,
                tipo: 'Ingreso',
                concepto: 'Balance de Apertura',
                id: `00000000-0000-0000-0000-${payload.length.toString().padStart(12, '0')}`
            });
            if (sheetName === 'BBVA PESOS') bbvaBalance = ini;
        }

        for (let i = 7; i < data.length; i++) {
            const row = data[i];
            if (!row || row.length < 3) continue;

            const dateStr = excelDate(row[0]);
            if (!dateStr) continue;

            const ing = parseValue(row[3]);
            const egr = parseValue(row[4]);
            
            if (ing === 0 && egr === 0 && !row[2]) continue;

            const monto = ing > 0 ? ing : egr;
            if (monto === 0) continue;

            const tipo = ing > 0 ? 'Ingreso' : 'Egreso';
            
            if (sheetName === 'BBVA PESOS') {
                bbvaBalance += (tipo === 'Ingreso' ? monto : -monto);
            }

            const ccStr = row[9]?.toString().trim() || '';
            const ccId = CC_MAP[ccStr] || null;

            payload.push({
                cuenta_id: accountId,
                fecha: dateStr,
                nombre_tercero: row[2]?.toString() || 'S/N',
                monto,
                tipo,
                factura: row[7]?.toString() || null,
                concepto: row[8]?.toString() || null,
                centro_costo_id: ccId,
                id: `00000000-0000-0000-0000-${payload.length.toString().padStart(12, '0')}`
            });
        }
    }

    console.log(`BBVA PESOS Calculated Balance: $${bbvaBalance.toLocaleString()}`);
    console.log(`Total rows to insert: ${payload.length}`);

    const BATCH = 100;
    for (let i = 0; i < payload.length; i += BATCH) {
        const { error } = await supabase.from('movimientos').insert(payload.slice(i, i + BATCH));
        if (error) console.error('Error batch:', error.message);
    }
    console.log('--- Sync Completed ---');
}

syncFinal();
